...
а не по департаментам. Категория = подсистема (A–E), подкатегория = процесс,
статья-сценарий лежит под процессом.

- **A. Стратегия и проектирование** — анализ рынка, проектирование процессов
- **B. Создание и поставка ценности** — продажи, сервис клиентам
- **C. Данные, аналитика и интеллект** — аналитика, ИИ-ассистенты и базы знаний
- **D. Обеспечение** — персонал (HR) и др. поддерживающие функции
- **E. Управление и контроль** — оперативное управление, контроль качества
...
└── site/                              # Next.js 16 приложение (всё живёт здесь)
    ...
    │   ├── strategy/                  # A. Стратегия и проектирование
    │   │   ├── market-analysis/
    │   │   └── process-design/
    │   ├── value-stream/              # B. Создание и поставка ценности
    │   │   ├── sales/
    │   │   └── client-service/
    │   ├── data/                      # C. Данные, аналитика и интеллект
    │   │   ├── analytics/
    │   │   └── ai-knowledge/
    │   ├── support/                   # D. Обеспечение
    │   │   └── hr/
    │   └── management/                # E. Управление и контроль
    │       ├── operations/
    │       └── quality/
    ├── lib/
    │   ├── tree.ts                    # Дерево категорий и подкатегорий (A–E)
...
- Таксономия процессная (ДИИП): категория = подсистема (A–E), подкатегория = процесс.
...
1. Выберите категорию (подсистему A–E) и подкатегорию (процесс) из дерева в
